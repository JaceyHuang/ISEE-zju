...
// 类名命名格式: X-format --> XFormInst
// 各指令类型下结构体名称:
// R-format --> bits
// B-format --> bits
// I-format --> fields
// 其中，lb为fields，slli为fields2
// S-format --> bits
// U-format --> bits
// J-format --> bits

#include <cassert>
#include <iso646.h> // 解决‘未定义标识符or、not’问题
...
IFormInst::encodeLb(unsigned rdv, unsigned rs1v, int offset)
{
  if (rdv > 31 or rs1v > 31)
    return false; // 寄存器最大索引为31

  if (offset >= (1 << 11) or -offset < -(1 << 11))
    return false;  // 立即数长度为12位
  
  fields.opcode = 0x03; // LB的opcode为3
  fields.rd = rdv & 0x1f; // 只取低5位，即小于32
  fields.funct3 = 0; // LB的funct3为0
  fields.rs1 = rs1v & 0x1f; // 同rd
...
IFormInst::encodeSlli(unsigned rd, unsigned rs1, unsigned shamt)
{
  ...
  if (shamt > 31)
    return false;  // 偏移量长度为5位，且只能为正数
  
  fields2.opcode = 0x13; // SLLI的opcode为13
  fields2.rd = rd & 0x1f; // 只取低5位，即小于32
  fields2.funct3 = 0x01; // SLLI的funct3为1
  fields2.rs1 = rs1 & 0x1f; // 同rd
  fields2.shamt = shamt & 0x1f; // 只取低5位，非符号数
  fields2.top7 = 0x00; // SLLI的高七位为0
...
BFormInst::encodeBeq(unsigned rs1v, unsigned rs2v, int imm)
{
  ...
  if (imm & 0x01)
    return false; // 立即数最低位必须为0

  if (imm >= (1 << 12) or -imm < -(1 << 12))
    return false; // 立即数长度为13位

  bits.opcode = 0x63; // 0b'1100011
  bits.imm11 = (imm >> 11) & 1; // 取立即数的第11位(记最低位为第0位)
  bits.imm4_1 = (imm >> 1) & 0x0f; // 取立即数的第1到第4位
  bits.funct3 = 0; // BEQ的funct3为0
  bits.rs1 = rs1v & 0x1f;
  bits.rs2 = rs2v & 0x1f;
...
SFormInst::encodeSb(unsigned rs1v, unsigned rs2v, int imm)
{
  ...
  bits.imm4_0 = imm & 0x1f; // 取imm的低5位
  bits.funct3 = 0; // SB的funct3为0
  bits.rs1 = rs1v & 0x1f;
  bits.rs2 = rs2v & 0x1f;
...
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wconversion"
  bits.imm11_5 = (imm >> 5) & 0x7f; // 取imm[11:5]，符号数
#pragma GCC diagnostic pop
...
JFormInst::encodeJal(uint32_t rdv, int offset)
{
  ...
  if (offset >= (1 << 20) or -offset < -(1 << 20))
    return false; // 立即数长度为21位

  bits.opcode = 0x6f ; // 0b'1101111
  bits.rd = rdv & 0x1f; 
  bits.imm19_12 = (offset >> 12) & 0xff; // 取imm[19:12]
