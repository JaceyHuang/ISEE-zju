...
def main():
    ...
    dataset_dir = home_dir/'..'/'..'/'data'/test_split/'imgs'
    save_dir = home_dir/'..'/'..'/'results'/'further_imp'/test_split
    ...
    for idx in range(1, n_imgs, step):
        ...
        weights = weighting_step(curr_img, particles, ref_wh, template, last_template, feature_type)
        ...
        # Decision of best bounding box location by weights
        top_k = 2     # select top_k particles according to weights and take the average
        max_kidx = np.argpartition(weights, -top_k)[-top_k:]    
        cx = np.sum(np.fromiter((particle.cx for particle in np.array(particles)[max_kidx]),float))/top_k
        cy = np.sum(np.fromiter((particle.cy for particle in np.array(particles)[max_kidx]),float))/top_k
        sx = np.sum(np.fromiter((particle.sx for particle in np.array(particles)[max_kidx]),float))/top_k
        sy = np.sum(np.fromiter((particle.sy for particle in np.array(particles)[max_kidx]),float))/top_k
        curr_particle = Particle(cx, cy, sx, sy, sigmas)
